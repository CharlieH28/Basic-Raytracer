...
public:
	...
	virtual glm::vec3 sampleRay(const Ray &ray, int depth = 0) override;
	glm::vec3 calculateAmbient(const Material& material);
	glm::vec3 calculateDiffuse(const Material& material, const glm::vec3& normal, const std::vector<Light>& lights);
	glm::vec3 calculateSpecular(const Material& material, const glm::vec3& normal, const std::vector<Light>& lights, const glm::vec3& viewDirection);
};