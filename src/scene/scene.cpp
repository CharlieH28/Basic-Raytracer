...
Scene Scene::shapeScene() {
	...
	// YOUR CODE GOES HERE
	// ...
	objects.push_back(make_shared<SceneObject>(make_shared<Plane>(vec3(0, -1, 0), vec3(0,1,0)), white));
	objects.push_back(make_shared<SceneObject>(make_shared<Disk>(vec3(1, 0, -5), vec3(1, 0, 1),1), white));
	objects.push_back(make_shared<SceneObject>(make_shared<Triangle>(vec3(3, -1, -5), vec3(3, 2, -5), vec3(4, -1, -5)), white));
